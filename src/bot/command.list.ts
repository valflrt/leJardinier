...
export default class CommandList {
	...
	constructor(...commands: ICommand[]) {
		...
		let sortSubcommands = (commands: ICommand[] | undefined) => {
			if (!commands) return;
			commands.forEach((command: ICommand) => {
				command.commands?.sort(filter);
				sortSubcommands(command.commands);
			});
		};
	...
	public fetch(messageContent: string): ICommand | undefined {
		if (messageContent.match(new RegExp(`^${config.prefix}`, "g")) !== null) {
			...
			let fetchCommand = (commands: ICommand[], remainingText: string): ICommand | undefined => {
				if (!commands) return;
				else {
					...
					if (command && command.commands) {
						remainingText = remainingText.replace(new RegExp(`^${command.name}(?!\\w+)`, "g"), "").trim();
						if (command.commands.find(
							(command: ICommand) =>
								remainingText.match(
									new RegExp(`^${command.name}(?!\\w+)`, "g")
								) !== null
						)) return fetchCommand(command.commands, remainingText);
						else return command;
