  ...
  execution: async (messageInstance) => {
    ...
    methods.sendTextEmbed(
      `With this command, you can register guilds and users.\n`
        ...
        .concat(`- ${inlineCode(`lj!register user`)}: register yourself`)
    );
  },
  commands: [
    new Command({
      ...
      execution: async (messageInstance) => {
        ...
        guildManager
          .add(message.guild!)
          .then(() => methods.sendTextEmbed(`Guild registered successfully`))
          .catch((err) => {
            console.log(err);
    ...
    new Command({
      ...
      execution: async (messageInstance) => {
        ...
        userManager
          .add(message.author!)
          .then(() => methods.sendTextEmbed(`User successfully registered`))
          .catch((err) => {
            console.log(err);
